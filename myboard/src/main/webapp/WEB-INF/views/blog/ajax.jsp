                                ...
                                <div class="comment-info">
                                    ...
                                    <div class="meta ps-0 d-flex justify-content-between">
                                        ...
                                        <c:if test="${loginHewon.id.equals(reply.rhid)}">
                                            <button onclick="deleteReply('${reply.ridx}', '${reply.rbidx}');" type="button" class="replybutton">삭제</button>
                                        </c:if>
                                    </div>

                                    <div class="reply">
                                        <c:if test="${reply.relevel==0}">
                                            <button type="button" class="replybutton" onclick="toggleReply('${reply.ridx}');" id="rebtn"> ↳ Reply </button>
                                        </c:if>
                                        <div id="replyDiv-${reply.ridx}" style="display: none;">
                                            <form name="reply-${reply.ridx}" method="post">
                                                ...
                                                <button type="button" class="butn-style2" onclick="addReply('${reply.ridx}');">등록</button>
                                            </form>
                                        </div>
                    ...
                    <c:if test="${not empty loginHewon}">
                        <div class="common-block">
                            ...
                            <form name="f" method="post">
                                ...
                                <button type="button" class="butn-style2" onclick="addReply('0');">Send Message</button>
                            </form>
                        </div>
                    </c:if>
                    <!-- end form -->
...
<script type="text/javascript">
    function addReply(replyRidx) {
        var replyDiv = document.getElementById("replyDiv-" + replyRidx);
        var comments = document.getElementById("comments-" + replyRidx).value;
    ...
    function deleteReply(replyRidx, replyBidx) {
    ...
    function toggleReply(replyRidx) {
        var replyDiv = document.getElementById("replyDiv-" + replyRidx);
